...
const DEFAULT_OVERLAY = 'rgba(30, 58, 138, 0.8), rgba(30, 64, 175, 0.7), rgba(59, 130, 246, 0.6)';
...
const Hero = () => {
  ...
  const [heroConfig, setHeroConfig] = useState<HeroConfig>({ mode: 'image', video_url: null, overlay: null });
    ...
    async function fetchHeroData() {
      try {
        ...
        if (slidesData && slidesData.length > 0) {
          setHeroImages(
            slidesData.map((s: { image_url: string }) => ({
              desktop: s.image_url,
              mobile: s.image_url
            }))
          );
  ...
  useEffect(() => {
    if (heroConfig.mode === 'video') return;
    const interval = setInterval(() => {
      setCurrentImageIndex((prev) => (prev + 1) % heroImages.length);
    }, 5000);
    return () => clearInterval(interval);
  }, [heroImages.length, heroConfig.mode]);

  const overlayGradient = heroConfig.overlay
    ? `linear-gradient(to right, ${heroConfig.overlay})`
    : `linear-gradient(to right, ${DEFAULT_OVERLAY})`;

  // VIDEO MODE
  if (heroConfig.mode === 'video' && heroConfig.video_url) {
    const isYouTube = heroConfig.video_url.includes('youtube.com') || heroConfig.video_url.includes('youtu.be');
    const isVimeo = heroConfig.video_url.includes('vimeo.com');
    ...
    return (
      <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          {isYouTube || isVimeo ? (
            <iframe
              src={`${heroConfig.video_url}?autoplay=1&mute=1&loop=1&controls=0&showinfo=0&rel=0&playsinline=1`}
    ...
    <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      {heroImages.map((image, index) => (
        <div
          ...
          style={{
            backgroundImage: `${overlayGradient}, url('${image.desktop}')`,
...
// Extracted hero content so it is shared between both modes
